...
def show_prediction_results(stock_code):
    ...
    # XGBoost结果
    xgb_csv_path = f"{stock_code}_XGBoost预测结果.csv"
    xgb_img_path = f"{stock_code}_预测图表.png"
    # 随机森林结果
    rf_csv_path = f"{stock_code}_随机森林预测结果.csv"
...
def prediction_page():
    ...
    with st.container():
        ...
        if query_button:
            ...
            try:
                # 读取沪深300成分股文件
                if os.path.exists('hs300_components.csv'):
                    components_df = pd.read_csv('hs300_components.csv')
                    # 显示完整的300个成分股
                    st.dataframe(components_df, use_container_width=True)
                    st.info(f"共 {len(components_df)} 只成分股，按权重排序。")
                else:
                    st.warning("未找到沪深300成分股文件，请确保 hs300_components.csv 文件存在。")
...
def evaluation_page():
    ...
    if not stock_code:
        ...
        return
    xgb_eval_path = f"{stock_code}_XGBoost评估.csv"
    rf_eval_path = f"{stock_code}_随机森林评估.csv"
